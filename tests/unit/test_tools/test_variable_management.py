...
class TestVariableWrite:
    ...
    @pytest.mark.asyncio
    async def test_dry_run_returns_diff_without_writing(self) -> None:
        expected = VariableWriteDryRunResult(
            action="scalar",
            variable_name="f_Kp_1",
            current_value=0.10,
            proposed_value=0.12,
        ...
        assert isinstance(result, VariableWriteDryRunResult)
        assert result["dry_run"] is True
        assert result["would_change"] is True
        assert result["current_value"] == 0.10
        assert result["proposed_value"] == 0.12

    @pytest.mark.asyncio
    async def test_dry_run_no_change_when_value_same(self) -> None:
        expected = VariableWriteDryRunResult(
            action="scalar",
            variable_name="f_Kp_1",
            current_value=0.12,
