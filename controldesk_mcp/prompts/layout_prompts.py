...
All layout-domain tools are covered:
  controldesk_layout_list, controldesk_layout_manage, controldesk_layout_discover, controldesk_layout_io_manage
    ...
    return [
        {
            ...
            "content": (
                ...
                f"**Prerequisite:** `controldesk_app_start_or_attach` must have been called and an "
                f"experiment must be open.\n\n"
                f"**Steps — execute in order:**\n\n"
                f"1. Call `controldesk_layout_list` to see all layouts in the active experiment "
                f"   (name, file path, open/active state, editing mode).\n"
                f"2. To create a new layout: call `controldesk_layout_manage` with "
                f"   `action='create', {name_arg}`.\n"
                f"3. To open an existing layout: call `controldesk_layout_manage` with "
                f"   `action='open', {name_arg}`.\n"
                f"4. To make a layout active (bring to foreground): call `controldesk_layout_manage` with "
                f"   `action='activate', {name_arg}`.\n"
                f"5. To set the editing mode (Design / Runtime / Hybrid): call `controldesk_layout_manage` "
                f"   with `action='configure', {name_arg}, editing_mode='Runtime'`.\n"
                f"6. To get full metadata of a layout: call `controldesk_layout_manage` with "
                f"   `action='get_info', {name_arg}`.\n"
                f"7. To save a layout: call `controldesk_layout_manage` with `action='save', {name_arg}`.\n"
                f"8. To close a layout: call `controldesk_layout_manage` with "
                f"   `action='close', {name_arg}, save_before_close=True`.\n"
                f"9. For export/import operations (layouts and connection files): "
                f"   first call `controldesk_layout_discover` to activate the `controldesk_layout_io_manage` tool, "
                f"   then:\n"
                f"   - Export active layout: `controldesk_layout_io_manage(action='export', "
                f"     export_path='C:/exports/my_layout.lax')`\n"
                f"   - Import layout: `controldesk_layout_io_manage(action='import', "
                f"     import_path='C:/exports/my_layout.lax')`\n"
                f"   - Export connection file: `controldesk_layout_io_manage("
                f"     action='export_connection_file', "
                f"     connection_file_path='C:/exports/connections.cdx')`\n"
                f"   - Import connection file: `controldesk_layout_io_manage("
